...
int within_array(int* int_array, int size, int* ptr) {
  long int start = (long int)int_array;
  long int end = (long int)(int_array + size - 1);
  long int p = (long int)ptr;
  int signofnum1 = (int)(end - p) >> 31;
  int signofnum2 = (int)(p - start) >> 31;

  return (signofnum1 + signofnum2) == 0;
}
...
void selection_sort(int arr[], int arr_length) {
  ...
  for (int i = 0; i < arr_length; i++) {
	  ...
	  for (int j = i + 1; j < arr_length; j++){
		  if(*(finalptr + min_index) > *(finalptr + j)) {
			  min_index = j;
		  }
	  }
	  temp1 = *(finalptr + i);
	  temp2 = *(finalptr + min_index);
	  *(finalptr + i) = temp2;
	  *(finalptr + min_index) = temp1;
}
}
